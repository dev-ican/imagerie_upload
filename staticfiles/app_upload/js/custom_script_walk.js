	...
	$.ajax({
		...
		if (data){
			...
			for (let i=1;i < count; i++){
				...
				if (Object.values(val_data)[i].dir == true){
					str_select += '<td><a name="' + Object.values(val_data)[i].url + '" id="walk_dir">Dossier : ' + Object.values(val_data)[i].nom + '</a></td><td>Nombre de fichiers ' + Object.values(val_data)[i].file + '</td><td>Nombre de dossiers ' + Object.values(val_data)[i].direct + '</td></tr>';
				} else {
					str_select += '<tr><td>' + Object.values(val_data)[i].nom + '</td><td><a href="/admin_page/upfiles/downOnce/' + Object.values(val_data)[i].url + '"><span button type="button" class="btn btn-primary">Télécharger</span></a></td></tr>';
				}
			}
			...
			document.getElementById('walk_tab').innerHTML = str_select;
			document.getElementById('walk_return').addEventListener("click",click_retour);
			document.querySelectorAll('#walk_dir').forEach(item => {
  				item.addEventListener("click",walk_rep);
			});
			//document.getElementById('walk_dir').addEventListener("click",walk_rep);
		//$('tr[id="walk_tab"]').html(str_select);
	}
	})
};

function walk_rep(event){
	var url_walk = "/admin_page/upfiles/walk_up/"
	var id_etude = $('a[id="id_etude"]').attr('name');
	var walk_url = $(this).attr('name');
	var csrftoken = $('input[name="csrfmiddlewaretoken"]').val();
	...
	$.ajax({
		...
		for (let i=1;i < count; i++){
			if (Object.values(val_data)[i].dir == true){
				str_select += '<tr><td><a name="' + Object.values(val_data)[i].url + '" id="walk_dir">Dossier : ' + Object.values(val_data)[i].nom + '</a></td><td>Nombre de fichiers ' + Object.values(val_data)[i].file + '</td><td>Nombre de dossiers ' + Object.values(val_data)[i].direct + '</td></tr>';
			} else {
				str_select += '<tr><td>' + Object.values(val_data)[i].nom + '</td><td><a href="/admin_page/upfiles/downOnce/' + Object.values(val_data)[i].url + '"><span button type="button" class="btn btn-primary">Télécharger</span></a></td></tr>';
			}
		}
