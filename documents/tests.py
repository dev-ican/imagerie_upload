...
#---------------------------------------------------------------------------------------------
	...
	def test_gestion_doc(self):
		...
		val_dict = {'user_current': id_user[0], 'etudes':id_etude[0].id, 'titre':"Test_doc_ajout", 'description':"Desc_Test_Doc_ajout", 'type':1, 'document':myfile}
		post_document = self.client.post(reverse('gestion'), data=val_dict)
	...
	def test_doc_edit(self):
		...
		files = settings.BASE_DIR + '\\documents\\test_img.png'
		myfile = open(files,'rb')
		val_dict = {'user_current': id_user[0], 'etudes':id_etude[0].id, 'titre':"Test_doc_edition", 'description':"Desc_Test_Doc_edition", 'type':1, 'document':myfile}
		post_document = self.client.post(reverse('gestion'), data=val_dict)
		result = post_document.context['resultat']
		for item in result:
			if item.titre == "Test_doc_edition":
				id_item = item.id
		edit_document = self.client.post(reverse('doc_edit', args=(id_item,)), {'etudes':id_etude[0].id,'titre':"Test_doc_EDITER", 'description':"Desc_Test_EDITER", 'type':1})
	...
	def test_doc_deleted(self):
		...
		files = settings.BASE_DIR + '\\documents\\test_img.png'
		myfile = open(files,'rb')
		val_dict = {'user_current': id_user[0], 'etudes':id_etude[0].id, 'titre':"Test_doc_deleted", 'description':"Desc_Test_Doc_deleted", 'type':1, 'document':myfile}
		post_document = self.client.post(reverse('gestion'), data=val_dict)
		result = post_document.context['resultat']
		for item in result:
			if item.titre == "Test_doc_deleted":
				id_item = item.id
		edit_document = self.client.post(reverse('doc_deleted', args=(id_item,)))
